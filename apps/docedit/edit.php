...
<script src="../../rsc/libs/moment.min.js"></script>
...
var docid = 0;
function main() {
//	databag = typeof localStorage.notes == 'undefined' || localStorage.notes == 'undefined' ? "" : localStorage.notes;
//	DOM("ED").innerHTML = databag;
	list_documents();
...
function list_documents() {
	...
	if(typeof localStorage.notes != 'undefined' && localStorage.notes != "") {
		idata = localStorage.notes.split("^~^");
		for(index in idata) {
			s = idata[index].split("~~");
	...
	H = "";
	for(did in ldata) {
		displayTime = "<div style='font-size:16px; padding-bottom:20px; color:#A0A0A0;'>" + moment().format("MMMM Do YYYY, h:mm:ss a") + "</div>";
		H = "<div style='padding-top:10px; padding-bottom:20px; padding-left:20px; padding-right:20px; margin-top:10px; border:1px solid #BABABA; border-radius:5px; background-color:#FFFFFF;' onclick='this.style.backgroundColor = \"#C0C0C0\"; window.setTimeout(function() {openDocument("+did+")},150)'>"+ displayTime + ldata[did] +  "</div>" + H;
	}
	DOM("LIST").innerHTML  = H;
	DOM("LIST").style.display  = "block";
	DOM("ED_block").style.display = "none";
...
function resize() {
	...
	if(_MODE  == _VIEW) {
		DOM("POINT").style.left = (w / 2 - 35) + "px";
	} else {
		DOM("POINT").style.left = (w - 75) + "px";	
	}
